...
url = f"https://api.airtable.com/v0/{base_id}/Draft_Insights?filterByFormula=" + urllib.parse.quote("{Draft ID} != ''")
req = urllib.request.Request(url, headers=headers)
try:
    with urllib.request.urlopen(req) as response:
        records = json.loads(response.read().decode('utf-8'))['records']
        for r in records:
            if r['id'] == 'rec8U1fZjQ0vYBOx7':
                print(f"Meeting ID: {r['fields'].get('Meeting ID')}")
